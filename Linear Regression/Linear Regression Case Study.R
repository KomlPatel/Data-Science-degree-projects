...
temp_num_data = data.frame(apply(temp_num_data, 2, FUN=miss_treat_num))
...
# Applied Anova test on  numeric cust_dbsset 
#colnames(mycust_dbs)

ANOVA_Linear <- aov(total_spent ~ ., data = temp_num_data)
...
require(GPArotation)
require(psych)
FA<-fa(r=corrm, 15, rotate="varimax", SMC=FALSE, fm="minres")   ### CONDUCTING FACTOR ANALYSIS
